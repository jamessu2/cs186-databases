...
def maxflow(bfs_max_iterations=float('inf'), flow_max_iterations=float('inf')):
    ...
    while True:
        ...
        while not sink_path_found:
            ...
            # Combine the `paths` table with the `edge` table to extend all paths
            # Hints: a JOIN would be helpful here. Also check the documentation to
            # see how array concatenation work in Postgres.
            db.execute("""
                    WITH edge_new(id, src, dst, capacity) AS (
                      SELECT edge.id, edge.src, edge.dst, edge.capacity
                      FROM paths, edge
                      WHERE edge.capacity != 0
                        AND paths.nodes[array_length(paths.nodes,1)] = edge.src
                        AND NOT (edge.dst = ANY (paths.nodes))
                    )

                    SELECT paths.path || edge_new.id AS path, paths.nodes || edge_new.dst AS nodes
                    INTO tmp
                    FROM paths INNER JOIN edge_new
                    ON paths.nodes[array_length(paths.nodes,1)] = edge_new.src;
        ...
        db.execute("""
            ...
            constraining_capacity(capacity) AS (
                SELECT MIN(edge.capacity)
                FROM path_edges INNER JOIN edge
                ON path_edges.path_edge = edge.id
            )
        ...
        db.execute("""
            WITH updates(id, new_capacity) AS (
                SELECT flow_to_route.edge_id, -1 * flow_to_route.flow
                FROM flow_to_route

                UNION

                SELECT flip_edge.reverse_id, flow_to_route.flow
                FROM flow_to_route INNER JOIN flip_edge
                ON flow_to_route.edge_id = flip_edge.forward_id
            )
            UPDATE edge
              SET capacity = capacity + updates.new_capacity
              FROM updates
