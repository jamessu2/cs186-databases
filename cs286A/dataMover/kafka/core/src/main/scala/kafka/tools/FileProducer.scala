...
object FileProducer {
  ...
  def main(args: Array[String]) {
    ...
    val FILENAME : String= config.inputFile
    val fileInputStream = new FileInputStream(new File(FILENAME));
    reader.init(fileInputStream, getReaderProps(config))
  ...
  class ProducerConfig(args: Array[String]) {
    ...
    val inputFileOpt = parser.accepts("input-file", "REQUIRED: The input file for produce messages to.")
      .withRequiredArg
