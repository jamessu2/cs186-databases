...
DashboardController.prototype.filterTransactionsByMapSelection = function () {
    ...
    function isSelectedState(input) {
        if ('state' in input && typeof(input.state) === 'string' && input.state.length === 2) {
            return that.usCashMap.hasStateInSelection(input.state);
        }
        else {
            return false;
        }
    ...
    var amounts_for_selected_state = this.allTransactions.filter(isSelectedState);
    return amounts_for_selected_state;
};
