...
USCashMap.prototype.render = function (data) {
    ...
    this.states.on('mouseover', function(d, e, p){
        ...
        var full_state_name = that.stateNameMap.get(d['state']);
        that.setInspectionInfo(full_state_name,d['total_amount']);

        this.style.fill = "blue";
        //this.style.fill = moneyColorScale(60050768);  // Return a hexcode
    });
    ...
    this.states.on('mouseout', function(d, e, p){
        ...
        // Update color to the original color (the original moneyColorScale color)
        this.style.fill = moneyColorScale(d['total_amount']);  // Return a hexcode
    });
