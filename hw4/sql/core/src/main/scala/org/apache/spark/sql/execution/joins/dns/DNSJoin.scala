...
trait DNSJoin {
  ...
  def hashJoin(input: Iterator[Row]): Iterator[Row] = {
    new Iterator[Row] {
      // IMPLEMENT ME
      var requests: ConcurrentHashMap[ Int, Row ] = new ConcurrentHashMap[ Int, Row ]()
      var responses: ConcurrentHashMap[ Int, Row ] = new ConcurrentHashMap[ Int, Row ]()

      var ipToRequestMap : ConcurrentHashMap[ Int, Int ] = new ConcurrentHashMap[ Int, Int ]()
      var requestCounter : Int = 0

      var outputQueue : Queue[ Row ] = new Queue[ Row ]()

      while( requests.size < requestBufferSize && input.hasNext ){
        makeRequest()
      }
      ...
      override def next() = {
        // IMPLEMENT ME
        if( hasNext ){ outputQueue.dequeue() } else { null }
      }
      ...
      override def hasNext() = {
        // IMPLEMENT ME
        //println( "begin hasNext with requestsHT size = " + requests.size() )
        while( outputQueue.isEmpty && !requests.isEmpty ){

          var requestKeys = requests.keySet()
          var requestKeysScala = requestKeys.take( requestKeys.size )

          for( requestKey <- requestKeysScala ){

            // Retreiving the key of a request (i.e. its IP address)
            // to obtain the address' request number
            var inputRow : Row = requests.get( requestKey )
            if( inputRow != null ){
              var inputIP : String = leftKeyGenerator( inputRow ).getString( 0 )
              var inputKey : Int = inputIP.hashCode()

              var responseKey = ipToRequestMap.get( inputKey )
              if( responses.containsKey( responseKey ) ){
                outputQueue.enqueue( 
                  new JoinedRow( requests.get( requestKey ), responses.get( responseKey ) ) )
      ...
      private def makeRequest() = {
        // IMPLEMENT ME
        var inputRow : Row = input.next()
        var inputIP : String = leftKeyGenerator( inputRow ).getString( 0 )
        var inputKey : Int = inputIP.hashCode()

        requestCounter = requestCounter + 1
        requests.put( requestCounter, inputRow )

        if( !ipToRequestMap.containsKey( inputKey ) ){
          ipToRequestMap.put( inputKey, requestCounter )
          DNSLookup.lookup( requestCounter, inputIP, responses, requests )
        }
      }
    }
  }
